...
void Game_Over()
{
    gotoxy(30,10);
    printf("Game Over");
    record.length = Snake_Len;
    record.level = Level;
    record.score = All_Score;
    record.speed = Wait_Time/10;
    record.tot = Apple_Num;
    fp = fopen("Record.txt", "a+");
    fprintf(fp, "\n%s\t%d\t%d\t%d\t0.%ds\t%d", record.name, record.score, record.level, record.length, record.speed, record.tot);
    Sleep(3000);
    system("pause>nul");
    exit(0);
}
