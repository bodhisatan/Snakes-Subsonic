...
bool isVirtualTailReachable_Regular()
{
    ...
    {
        setPosition(46, 11);
        printf("VirtualSnake != Snake, enter to exit");
        getchar();
        getchar();
        exit(-1);
    }
...
bool isSafe(SnakeNode NewHead)
{
    ...
    {
        setPosition(46, 16);
        printf("VirtualSnakeHead != NewHead");
        getchar();
        exit(-105);																//异常情况，调试使用
    }
    for (int i = 0; i < 4; i++)
...
int getDepth(SnakeNode temp)
{
    ...
    for (list<SnakeNode>::iterator it = Snake.begin(); it != Snake.end(); it++)
    {
        ...
        {
            setPosition(44, 4);
            printf("iterator Error\t%d\t%d", (*it).x, (*it).y);
            getchar();
            exit(-1);
        }
